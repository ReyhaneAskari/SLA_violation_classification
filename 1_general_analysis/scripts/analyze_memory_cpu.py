...
task_events_csv_colnames = [
    ...
    'disk', 'restriction']
task_event_df = read_csv(path.join('task_events', 'part-00499-of-00500.csv.gz'),
                         header=None, index_col=False,
                         compression='gzip', names=task_events_csv_colnames)

seed(83)
sample_moments = sorted(sample(xrange(250619902823), 25061990))
...
for fn in sorted(listdir('task_events')):
    ...
    task_events_df = read_csv(fp, header=None, index_col=False,
                              compression='gzip', 
...
samples_df.to_csv('newsamples_df1.csv')
tasks_df.to_csv('newtasks_df1.csv')
...
for fn in sorted(listdir('machine_events')):
    ...
    machine_events_df = read_csv(
        fp, header=None, index_col=False, compression='gzip', names=machine_events_csv_colnames)
    for index, event in machine_events_df.iterrows():
        ...
        if current_sample_moment is not None and event['time'] > current_sample_moment:
            ...
            samples_dicts[current_sample_moment].update({'cpu_available': np.nansum(tmp_machines_df['cpu']), 
                                                         'mem_available': np.nansum(tmp_machines_df['mem'])})
            try:
                current_sample_moment = next(sample_moments_iterator)
            except StopIteration:
                current_sample_moment = None
...
samples_df.to_csv('newsamples_df2.csv')
machines_df.to_csv('newmachines_df.csv')
...
samples_df.to_csv('newsamples_df4.csv')
machines_df.to_csv('newtasks_df4.csv')
